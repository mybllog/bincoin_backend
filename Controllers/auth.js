...
const Users = db.user
const Token = db.confirmation_token
...
const Register = async(req, res) =>{
    try {
        ...
        const userId = insertResult.id; // Use insertId to get the generated user ID
            console.log(userId,">>>>>>>>>>>>>")
...
const verifyRegistration = async (req, res) => {
    const { token} = req.query; // Check if it should be req.body or req.query
      console.log({token},333333333)
    try {
        const result = await Token.findOne({
            where:{token}
        })

       // const [result] = await db.query('SELECT * FROM confirmation_token WHERE token = ?' ,[token]);
        ...
        const expiredTokenDate = result.expirationDate;
        const currentDate = new Date();

        // Ensure userId is defined
        const userId = result.user_id;
            console.log(userId,555555555)
        if (expiredTokenDate && expiredTokenDate <= currentDate) {
           ...
           // await db.query('UPDATE confirmation_token SET token = ?, expireDate = ?, user_id = ? WHERE id = ?', [newToken, expirationDate, userId, userResult.id]);
        ...
        //await db.query('UPDATE user SET token = ?, status = ? WHERE id = ?', [token, status, userId]);
...
const Login = async (req, res) => {
    try {
      ...
      await Users.update({
        ...
        refreshExpire:expireDate,
        
      } , {where:{id:userId}})
        //await db.query('UPDATE user SET refreshToken = ?, refreshExpire = FROM_UNIXTIME(?) WHERE id = ?', [refreshToken, expireDate, userId]);
...
const verifyRefreshToken = async (req, res) => {
    ...
    try {
        ...
        //const user = await db.query('SELECT * FROM user WHERE refreshToken=?', [refreshToken])
...
const resetPassword = async (req, res) => {
    try {
        ...
        //const [result] = await db.query('SELECT * FROM user WHERE email = ?', [decodedToken.email]);
...
const updatePassword = async(req, res) =>{
    try {
        ...
        //await db.query('UPDATE user SET password = ? WHERE id = ?', [hashedPassword, userId]); // Fix the query
        ...
        Users.update(
            {
                ...
                [Sequelize.literal(setClause)]: Object.values(updatedFields),
            },
            {
                where: { id: userId }  // The condition to match the rows
            }
        )
        .then(([affectedRows]) => {
            if (affectedRows === 0) {
    ...
    const updatedUser = req.body;
     try {
        const [result] = await Users.update(
            updatedUser,  // The updated data
            {
                where: {
                    id: userId  // The condition to match the rows
                }
            }
        );
        
     //const [result] = db.query('UPDATE user SET ? WHERE id = ?', [updatedUser, userId])
                if (result.affectedRows === 0) {
