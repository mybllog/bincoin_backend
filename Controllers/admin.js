...
const adminSignup = async (req, res, next) => {
  ...
  try {
    ...
    const [userResult] = await db.query('SELECT id, role, super, status FROM user WHERE email = ?', [email]);
    ...
    if (req.userId && req.userId.role === 'admin') {
      ...
      const updatedRows = await db.query('UPDATE user SET role = ?, super = ? WHERE id = ?', [role, isSuperAdmin, user.id]);
